...
export default function Menu() {
  ...
  return (
    <div
      className={`
        min-h-[100dvh] bg-black text-white overflow-y-auto
        transition-opacity duration-200 ease-out
        ${closing ? 'opacity-0' : 'opacity-100'}
      `}
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 sm:px-6 md:px-8 lg:px-12 h-14 md:h-16 border-b border-white/[0.04]">
        <button
          onClick={handleClose}
        ...
        <button
          onClick={handleClose}
          className="p-2 -mr-2 hover:bg-white/[0.04] rounded-lg transition-colors"
        >
      ...
      <div className="px-4 sm:px-6 md:px-8 lg:px-12 py-6 md:py-8 lg:py-10">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-2 sm:gap-3 md:gap-4 max-w-[1400px] mx-auto">
          {items.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[11px] sm:text-[12px] text-white/70 text-center leading-tight font-medium">
                  {item.title}
                </span>
                <span className="text-[9px] sm:text-[10px] text-white/25 text-center mt-1.5 leading-tight">
                  {item.subtitle}
                </span>
              </button>
            );
